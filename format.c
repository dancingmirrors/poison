/*
 * Copyright (C) 2006 Antti Nyk�nen <aon@iki.fi>
 *
...
static void fmt_name(rp_window_elem *win_elem, struct sbuf *buf)
{
    sbuf_copy(buf, window_name(win_elem->win));
}

static void fmt_number(rp_window_elem *win_elem, struct sbuf *buf)
{
    /* Window numbering disabled - output nothing */
    sbuf_copy(buf, "");
}

static void fmt_resname(rp_window_elem *win_elem, struct sbuf *buf)
{
...
static void fmt_resclass(rp_window_elem *win_elem, struct sbuf *buf)
{
...
static void fmt_status(rp_window_elem *win_elem, struct sbuf *buf)
{
...
static void fmt_windowid(rp_window_elem *elem, struct sbuf *buf)
{
    sbuf_printf_concat(buf, "%ld", (unsigned long) elem->win->w);
}

static void fmt_height(rp_window_elem *elem, struct sbuf *buf)
{
    sbuf_printf_concat(buf, "%d", elem->win->height);
}

static void fmt_width(rp_window_elem *elem, struct sbuf *buf)
{
    sbuf_printf_concat(buf, "%d", elem->win->width);
}

static void fmt_incheight(rp_window_elem *elem, struct sbuf *buf)
{
    int height;
...
static void fmt_incwidth(rp_window_elem *elem, struct sbuf *buf)
{
    int width;
...
static void fmt_gravity(rp_window_elem *elem, struct sbuf *buf)
{
    sbuf_copy(buf, wingravity_to_string(elem->win->gravity));
}

static void fmt_screen(rp_window_elem *elem, struct sbuf *buf)
{
    sbuf_printf_concat(buf, "%d", elem->win->vscreen->screen->screen_num);
}

static void fmt_xrandrscreen(rp_window_elem *elem, struct sbuf *buf)
{
    sbuf_printf_concat(buf, "%d",
                       elem->win->vscreen->screen->xrandr.output);
}

static void fmt_transient(rp_window_elem *elem, struct sbuf *buf)
{
    if (elem->win->transient)
        sbuf_concat(buf, "Transient");
}

static void fmt_maxsize(rp_window_elem *elem, struct sbuf *buf)
{
    if (elem->win->hints->flags & PMaxSize)
        sbuf_concat(buf, "Maxsize");
}

static void fmt_pid(rp_window_elem *elem, struct sbuf *buf)
{
