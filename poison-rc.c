...
static bool parse_color(const char *value, float color[static 4]) {
    if (!value) {
        return false;
    }
    if (*value == '#') {
    ...
    char buffer[9];
    if (len == 3) {
        buffer[0] = buffer[1] = value[0];
        buffer[2] = buffer[3] = value[1];
        buffer[4] = buffer[5] = value[2];
        buffer[6] = buffer[7] = 'f';
        buffer[8] = '\0';
    } else {
    ...
    unsigned long rgba = strtoul(buffer, NULL, 16);
...
void poison_config_init(struct poison_config *config) {
    ...
    config->decorations_enabled = 1;
    config->decoration_border_width = POISON_DECORATION_BORDER_WIDTH;
    config->decoration_title_height = POISON_DECORATION_TITLE_AUTO;
    config->decoration_respect_client = 1;
    config->decoration_font = NULL;
    parse_color(POISON_DECORATION_ACTIVE_COLOR, config->decoration_active_color);
    parse_color(POISON_DECORATION_INACTIVE_COLOR,
                config->decoration_inactive_color);
    parse_color(POISON_DECORATION_ACTIVE_BORDER_COLOR,
                config->decoration_active_border_color);
    parse_color(POISON_DECORATION_INACTIVE_BORDER_COLOR,
                config->decoration_inactive_border_color);
    parse_color(POISON_DECORATION_ACTIVE_TEXT_COLOR,
                config->decoration_active_text_color);
    parse_color(POISON_DECORATION_INACTIVE_TEXT_COLOR,
                config->decoration_inactive_text_color);
        ...
        if (sep) {
            *sep = '\0';
            ...
            if (strcmp(key, "padding") == 0) {
                ...
                long val = strtol(value, &endptr, 10);
                if (*endptr == '\0' && (val == 0 || val == 1)) {
                    config->decorations_enabled = (int)val;
                    wlr_log(WLR_INFO, "Config: decorations set to %d.", config->decorations_enabled);
                }
            } else if (strcmp(key, "decoration_respect_client") == 0) {
                char *endptr;
                long val = strtol(value, &endptr, 10);
                if (*endptr == '\0' && (val == 0 || val == 1)) {
                    config->decoration_respect_client = (int)val;
                    wlr_log(WLR_INFO, "Config: decoration_respect_client set to %d.", config->decoration_respect_client);
                }
            } else if (strcmp(key, "decoration_border_width") == 0) {
                char *endptr;
                long val = strtol(value, &endptr, 10);
                if (*endptr == '\0' && val >= 0 && val <= 100) {
                    config->decoration_border_width = (int)val;
                    wlr_log(WLR_INFO, "Config: decoration_border_width set to %d.", config->decoration_border_width);
                }
            } else if (strcmp(key, "decoration_title_height") == 0) {
                if (strcmp(value, "auto") == 0) {
                    config->decoration_title_height = POISON_DECORATION_TITLE_AUTO;
                    wlr_log(WLR_INFO, "Config: decoration_title_height set to auto.");
                } else {
                    char *endptr;
                    long val = strtol(value, &endptr, 10);
                    if (*endptr == '\0' && val >= 0 && val <= 200) {
                        config->decoration_title_height = (int)val;
                        wlr_log(WLR_INFO, "Config: decoration_title_height set to %d.", config->decoration_title_height);
                    }
                }
            } else if (strcmp(key, "decoration_font") == 0) {
                char *dup = strdup(value);
                if (dup) {
                    if (config->decoration_font) {
                        free(config->decoration_font);
                    }
                    config->decoration_font = dup;
                    wlr_log(WLR_INFO, "Config: decoration_font set to '%s'.", config->decoration_font);
                } else {
                    wlr_log(WLR_ERROR, "Config: failed to allocate memory for decoration_font!");
                }
            } else if (strcmp(key, "decoration_active_color") == 0 ||
                       strcmp(key, "decoration_inactive_color") == 0 ||
                       strcmp(key, "decoration_active_border_color") == 0 ||
                       strcmp(key, "decoration_inactive_border_color") == 0 ||
                       strcmp(key, "decoration_active_text_color") == 0 ||
                       strcmp(key, "decoration_inactive_text_color") == 0) {
                float *target =
                    (strcmp(key, "decoration_active_color") == 0)
                    ? config->decoration_active_color
                    : (strcmp(key, "decoration_inactive_color") == 0)
                    ? config->decoration_inactive_color
                    : (strcmp(key, "decoration_active_border_color") == 0)
                    ? config->decoration_active_border_color
                    ...
                    ? config->decoration_active_text_color
                    : config->decoration_inactive_text_color;
                if (parse_color(value, target)) {
                    wlr_log(WLR_INFO, "Config: %s set to '%s'.", key, value);
                } else {
                    wlr_log(WLR_ERROR, "Config: '%s' is not a valid color for %s.", value, key);
                }
                ...
                if (!valid) {
                    wlr_log(WLR_ERROR, "Config: %s wants absolute device paths "
                                       "or DRM driver names.",
                            key);
                } else {
                    char *dup = strdup(value);
