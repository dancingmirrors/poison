...
static struct wlr_box hsplit_full_tile_xwayland(
    ...
    return (struct wlr_box){
        .x = output_box.x + ep + insets.left,
        .y = output_box.y + ep + insets.top,
        .width = output_box.width - 2 * ep - insets.left - insets.right,
        .height = output_box.height - 2 * ep - insets.top - insets.bottom,
    };
}

static void hsplit_restore_tile_xwayland(struct poison_server *server,
    ...
    struct wlr_box tile = hsplit_full_tile_xwayland(server, view);
    if (tile.width <= 0 || tile.height <= 0) {
        return;
    }
    wlr_xwayland_surface_configure(view->xwayland_surface, tile.x, tile.y,
...
struct wl_list *hsplit_evict(struct poison_toplevel *old) {
    ...
    if (output_box.width > 2 * effective_padding &&
        ...
        poison_toplevel_apply_size(old,
                                   output_box.width - 2 * effective_padding,
                                   output_box.height - 2 * effective_padding);
    }
    old->maximized = false;
    poison_toplevel_set_tiling(old, true, true);
    return pos;
}
...
struct wl_list *hsplit_evict_xwayland(struct poison_xwayland_view *old) {
    ...
    hsplit_restore_tile_xwayland(server, old);
    return pos;
}
...
void hsplit_place(struct poison_toplevel *toplevel, struct wl_list *pos) {
    ...
    poison_toplevel_set_tiling(toplevel, true, false);
...
void hsplit_place_xwayland(struct poison_xwayland_view *xw,
    ...
    if (xw->xwayland_surface) {
        wlr_xwayland_surface_set_maximized(xw->xwayland_surface, false, false);
    }
    if (xw->floating) {
...
void apply_hsplit_layout(struct poison_server *server) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (wins[i].xdg) {
            ...
            poison_toplevel_apply_box(tl, x, y, slot_width, height);
            wlr_scene_node_raise_to_top(&tl->scene_tree->node);
        } else {
            struct poison_xwayland_view *xwv = wins[i].xw;
            struct poison_frame_insets insets =
                poison_xwayland_view_insets(xwv);
            struct wlr_box slot = {
                .x = x + insets.left,
                .y = y + insets.top,
                .width = slot_width - insets.left - insets.right,
            ...
            wlr_xwayland_surface_configure(xwv->xwayland_surface, slot.x, slot.y,
                                           slot.width, slot.height);
            if (xwv->scene_tree) {
                wlr_scene_node_set_position(&xwv->scene_tree->node, slot.x,
                                            slot.y);
                wlr_scene_node_raise_to_top(&xwv->scene_tree->node);
            }
            set_xwayland_view_clip(xwv, &slot);
            poison_decoration_update_xwayland(xwv);
        }
    }
    ...
    if (count == 1) {
        ...
        if (xdg_count == 1) {
            ...
            if (restore_toplevel_fullscreen_from_hsplit(t)) {
                ...
                t->floating = false;
                t->maximized = false;
                poison_toplevel_set_tiling(t, true, true);
                if (output_box.width > 2 * ep && output_box.height > 2 * ep) {
                    poison_toplevel_apply_size(t,
                                               output_box.width - 2 * ep, output_box.height - 2 * ep);
                }
                poison_toplevel_apply_position(t,
            ...
            if (restore_xwayland_fullscreen_from_hsplit(xw)) {
                ...
                xw->floating = false;
                hsplit_restore_tile_xwayland(server, xw);
            }
        }
        server->hsplit_active = false;
        server->hsplit_focused_slot = -1;
...
void hsplit_only(struct poison_server *server) {
    ...
    if (focused_xdg) {
        ...
        if (restore_toplevel_fullscreen_from_hsplit(focused)) {
            ...
            poison_toplevel_set_tiling(focused, true, true);
            if (output_box.width > 2 * effective_padding &&
                output_box.height > 2 * effective_padding) {
                poison_toplevel_apply_size(focused,
                                           output_box.width - 2 * effective_padding,
                                           output_box.height - 2 * effective_padding);
            }
            poison_toplevel_apply_position(focused,
                                           output_box.x + effective_padding,
                                           output_box.y + effective_padding);
        }
        ...
        if (restore_xwayland_fullscreen_from_hsplit(focused_xwayland)) {
            ...
            hsplit_restore_tile_xwayland(server, focused_xwayland);
        }
