...
static unsigned char *create_gradient(struct wallpaper_state *state)
{
...
static int
parse_geometry(const char *geom, int *x, int *y, int *w, int *h,
               int *flags)
{
    unsigned int uw, uh;
            ...
            const char *image_file, int is_emblem)
{
    ...
    if (state->scale_height_percent > 0)
        dest_h =
            (int) (state->height * state->scale_height_percent / 100.0);
                          ...
                          int width, int height)
{
    ...
    pixmap =
        XCreatePixmap(dpy, RootWindow(dpy, screen), width, height, depth);
...
static void set_root_pixmap(Display *dpy, int screen, Pixmap pixmap)
{
    ...
    if (XGetWindowProperty(dpy, root, esetroot_pmap_id,
                           0L, 1L, False, XA_PIXMAP, &type, &format,
                           &nitems, &bytes_after,
