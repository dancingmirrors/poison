/*
 * Copyright © 2019 Joshua Stein <jcs@jcs.org>
 *
...
int vscreens_resize(int n)
{
    ...
    if (n < defaults.vscreens) {
        list_for_each_entry(scr, &rp_screens, node) {
            list_for_each_safe_entry(cur, iter, tmp, &scr->vscreens, node) {
                if (cur->number < n)
                    continue;

                if (scr->current_vscreen == cur)
                    set_current_vscreen(screen_find_vscreen_by_number(scr,
                                                                      0));
...
void vscreen_move_window(rp_vscreen *to, rp_window *w)
{
...
rp_vscreen *vscreen_next_vscreen(rp_vscreen *vscreen)
{
    return list_next_entry(vscreen->screen->current_vscreen,
                           &vscreen->screen->vscreens, node);
}

rp_vscreen *vscreen_prev_vscreen(rp_vscreen *vscreen)
{
                                        ...
                                        int exact_match)
{
    ...
    if (exact_match) {
        ...
        list_for_each_entry(cur, &s->vscreens, node) {
            if (cur->name && str_comp(name, cur->name, strlen(name)))
                return cur;
        }
    }
...
static int vscreen_in_list(struct list_head *h, rp_window_elem *w)
{
    rp_window_elem *cur;
...
rp_window *vscreen_next_window(rp_vscreen *v, rp_window *win)
{
    ...
    for (cur = list_next_entry(we, &v->mapped_windows, node);
         cur != we; cur = list_next_entry(cur, &v->mapped_windows, node)) {
        if (cur->win->sticky_frame != EMPTY &&
            (!f || (cur->win->sticky_frame != f->number)))
            continue;
...
rp_window *vscreen_prev_window(rp_vscreen *v, rp_window *win)
{
    ...
    for (cur = list_prev_entry(we, &v->mapped_windows, node);
         cur != we; cur = list_prev_entry(cur, &v->mapped_windows, node)) {
        if (cur->win->sticky_frame != EMPTY &&
            (!f || (cur->win->sticky_frame != f->number)))
            continue;
