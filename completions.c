...
static void completions_update(rp_completions *c, char *partial)
{
...
/*
 * Return true if completion is an alternative for partial string, given the
 * style used.
 */
static int
...
static char *completions_prev_match(rp_completions *c)
{
    struct sbuf *cur;
    ...
    for (cur = list_prev_entry(c->last_match, &c->completion_list, node);
        ...
        if (completions_match(c, sbuf_get(cur), c->partial)) {
            /*
             * We found a match so update our last_match pointer
