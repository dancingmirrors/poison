             ...
             char *keysym_name, int len, int ignore_bad_mods)
{
    ...
    *mod &= (rp_modifier_info.meta_mod_mask
             | rp_modifier_info.alt_mod_mask
             | rp_modifier_info.hyper_mod_mask
             | rp_modifier_info.super_mod_mask | ControlMask | shift);
...
int
read_key(KeySym *keysym, unsigned int *modifiers, char *keysym_name,
         int len)
{
    XEvent ev;
    int nbytes;
...
static void update_input_window(rp_screen *s, rp_input_line *line)
{
    ...
    rp_draw_string(s, s->input_window, STYLE_NORMAL,
                   ...
                   line->prompt, -1, NULL, NULL);

    rp_draw_string(s, s->input_window, STYLE_NORMAL,
                   defaults.bar_x_padding + prompt_width,
                   defaults.bar_y_padding + FONT_ASCENT(s),
                   line->buffer, line->length, NULL, NULL);
                     ...
                     enum completion_styles style, completion_fn compl_fn)
{
    ...
    while (!done) {
        ...
        case EDIT_DONE:
            ...
            break;
        default:
            errx(1, "unhandled input status %d; this is a *BUG*", status);
        }
    }
