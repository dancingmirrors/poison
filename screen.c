...
rp_screen *find_screen_by_attr(XWindowAttributes attr)
{
    rp_screen *cur;
    ...
    list_for_each_entry(cur, &rp_screens, node) {
        if (attr.x >= cur->left &&
            ...
            attr.y >= cur->top && attr.y <= cur->top + cur->height)
            return cur;
    }
...
static int screen_cmp(void *priv, struct list_head *a, struct list_head *b)
{
...
static void screen_set_numbers(void)
{
    rp_screen *cur;
...
static void screen_select_primary(void)
{
...
static void init_global_screen(rp_global_screen *s)
{
    XColor color, junk;
    int screen_num;
...
static void init_rat_cursor(rp_screen *s)
{
...
static void init_screen(rp_screen *s)
{
    ...
    s->bar_window = XCreateSimpleWindow(dpy, s->root, 0, 0, 1, 1,
                                        ...
                                        rp_glob_screen.bar_bordercolor,
...
static int is_rp_window_for_given_screen(Window w, rp_screen *s)
{
    if (w != s->key_window &&
        w != s->bar_window &&
        w != s->input_window &&
        w != s->frame_window && w != s->help_window)
        return 0;
    return 1;
}
...
char *screen_dump(rp_screen *screen)
{
    ...
    sbuf_printf_concat(s, "%d %d %d %d %d %d", screen->number, screen->left, screen->top, screen->width, screen->height, (rp_current_screen == screen) ? 1 : 0  /* is current? */
        );
...
static void screen_remove_current(void)
{
...
void screen_update_frames(rp_screen *s)
{
    ...
    list_for_each_entry(v, &s->vscreens, node) {
        list_for_each_entry(f, &v->frames, node) {
            ...
            if (frame_right_screen_edge(f) || (f->edges & EDGE_RIGHT))
                f->width = screen_right(v->screen) - f->x;

            if (frame_bottom_screen_edge(f) || (f->edges & EDGE_BOTTOM))
