...
static void get_mouse_position(rp_window *win, int *mouse_x, int *mouse_y)
{
...
rp_window *add_to_window_list(rp_screen *s, Window w)
{
    ...
    if (child_info) {
        ...
        if (!child_info->window_mapped) {
            rp_frame *frame =
                vscreen_find_frame_by_frame(child_info->vscreen,
                                            child_info->frame);
...
static void save_mouse_position(rp_window *win)
{
    Window root_win, child_win;
    int root_x, root_y;
    unsigned int mask;
...
static rp_frame *find_frame_non_dedicated(rp_vscreen *current_vscreen)
{
    rp_frame *cur;
    rp_screen *screen;
    ...
    list_for_each_entry(screen, &rp_screens, node) {
        ...
        list_for_each_entry(cur, &screen->current_vscreen->frames, node) {
            if (!cur->dedicated)
                return cur;
        }
    }
...
void window_full_screen(rp_window *win)
{
    ...
    rp_current_screen->full_screen_win = win;
    win->full_screen = 1;
    set_atom(win->w, _net_wm_state, XA_ATOM, &_net_wm_state_fullscreen, 1);
    maximize(win);
}
