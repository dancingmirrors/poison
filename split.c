...
static void update_last_access(rp_frame *frame)
{
...
static void create_initial_frame(rp_vscreen *vscreen)
{
    rp_frame *frame;
 ...
 * horizontally.
 */
static void split_frame(rp_frame *frame, int way, int pixels)
{
    rp_vscreen *v;
    rp_window *win;
    rp_frame *new_frame;
...
void resize_shrink_to_window(rp_frame *frame)
{
    ...
    resize_frame_horizontally(frame,
                              ...
                              (defaults.gap * 2) - frame->width);
    resize_frame_vertically(frame,
                            win->height + (win->border * 2) +
                            (defaults.gap * 2) - frame->height);
}
    ...
    list_for_each_entry(cur, &v->frames, node) {
        ...
        if ((*c1) (cur) == (*c3) (frame)) {
            ...
            if (diff > 0 && abs((*c3) (cur) - (*c1) (cur)) - diff <=
                (defaults.window_border_width * 2) + (defaults.gap * 2))
                return -1;
...
static int resize_frame_bottom(rp_frame * frame, rp_frame * pusher,
                               int diff);
static int resize_frame_top(rp_frame * frame, rp_frame * pusher, int diff);
static int resize_frame_left(rp_frame * frame, rp_frame * pusher,
                             int diff);
static int resize_frame_right(rp_frame * frame, rp_frame * pusher,
                              int diff);
...
static int resize_frame_right(rp_frame *frame, rp_frame *pusher, int diff)
{
    return resize_frame(frame, pusher, diff,
                        ...
                        frame_resize_right, frame_resize_left,
                        resize_frame_left);
}
...
static int resize_frame_left(rp_frame *frame, rp_frame *pusher, int diff)
{
    return resize_frame(frame, pusher, diff,
                        ...
                        frame_resize_left, frame_resize_right,
                        resize_frame_right);
}
...
static int resize_frame_top(rp_frame *frame, rp_frame *pusher, int diff)
{
    return resize_frame(frame, pusher, diff,
                        ...
                        frame_resize_up, frame_resize_down,
                        resize_frame_bottom);
}
...
static int resize_frame_bottom(rp_frame *frame, rp_frame *pusher, int diff)
{
    return resize_frame(frame, pusher, diff,
                        ...
                        frame_resize_down, frame_resize_up,
                        resize_frame_top);
}
...
static int frame_is_below(rp_frame *src, rp_frame *frame)
{
    if (frame->y > src->y)
        return 1;
    return 0;
...
static int frame_is_above(rp_frame *src, rp_frame *frame)
{
    if (frame->y < src->y)
        return 1;
    return 0;
...
static int frame_is_left(rp_frame *src, rp_frame *frame)
{
    if (frame->x < src->x)
        return 1;
    return 0;
...
static int frame_is_right(rp_frame *src, rp_frame *frame)
{
    if (frame->x > src->x)
        return 1;
    return 0;
...
static int total_frame_area(rp_vscreen *v)
{
    int area = 0;
    rp_frame *cur;
...
static int frames_overlap(rp_frame *f1, rp_frame *f2)
{
    if (f1->x >= f2->x + f2->width
        || f1->y >= f2->y + f2->height
        || f2->x >= f1->x + f1->width || f2->y >= f1->y + f1->height) {
        return 0;
    }
    return 1;
}
...
static int frame_overlaps(rp_frame *frame)
{
    rp_vscreen *v;
    rp_frame *cur;
