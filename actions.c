...
static void set_var_free(struct set_var *var)
{
    if (var == NULL)
        return;
...
static void init_set_vars(void)
{
    ...
    add_set_var("winaddcurvscreen", set_winaddcurvscreen, 1, "",
                arg_NUMBER);
}
...
static void user_command_free(struct user_command *cmd)
{
    if (cmd == NULL)
        return;
...
void init_user_commands(void)
{
    ...
    add_command("addhook", cmd_addhook, 2, 2, 2,
                "Hook: ", arg_HOOK, "Command: ", arg_COMMAND);
    add_command("alias", cmd_alias, 2, 2, 2,
                "Alias: ", arg_STRING, "Command: ", arg_COMMAND);
    add_command("banish", cmd_banish, 0, 0, 0);
    add_command("chdir", cmd_chdir, 1, 0, 0, "Dir: ", arg_REST);
    ...
    add_command("colon", cmd_colon, 1, 0, 0, "", arg_REST);
    ...
    add_command("dedicate", cmd_dedicate, 1, 0, 0, "", arg_NUMBER);
    add_command("definekey", cmd_definekey, 3, 3, 3,
                "Keymap: ", arg_KEYMAP,
                "Key: ", arg_KEY, "Command: ", arg_COMMAND);
    add_command("delete", cmd_delete, 0, 0, 0);
    add_command("delkmap", cmd_delkmap, 1, 1, 1, "Keymap: ", arg_KEYMAP);
    add_command("describekey", cmd_describekey, 1, 1, 1,
                "Keymap: ", arg_KEYMAP);
    add_command("echo", cmd_echo, 1, 1, 1, "Echo: ", arg_RAW);
    add_command("escape", cmd_escape, 1, 1, 1, "Key: ", arg_KEY);
    ...
    add_command("exec", cmd_exec, 1, 1, 1, "/bin/sh -c ", arg_SHELLCMD);
    add_command("execa", cmd_execa, 1, 1, 1, "/bin/sh -c ", arg_SHELLCMD);
    ...
    add_command("execv", cmd_execv, 2, 2, 2,
                ...
                "/bin/sh -c ", arg_SHELLCMD);
    add_command("execw", cmd_execw, 1, 1, 1, "/bin/sh -c ", arg_SHELLCMD);
    add_command("term", cmd_term, 0, 0, 0);
    add_command("fdump", cmd_fdump, 1, 0, 0, "", arg_NUMBER);
    add_command("float", cmd_float, 1, 1, 0, "Float: ", arg_REST);
    ...
    add_command("frestore", cmd_frestore, 1, 1, 1, "Frames: ", arg_REST);
    add_command("fselect", cmd_fselect, 1, 1, 1, "", arg_FRAME);
    add_command("getenv", cmd_getenv, 1, 1, 1, "Variable: ", arg_STRING);
    add_command("getsel", cmd_getsel, 0, 0, 0);
    add_command("gravity", cmd_gravity, 1, 0, 0, "Gravity: ", arg_GRAVITY);
    add_command("help", cmd_help, 1, 0, 0, "Keymap: ", arg_KEYMAP);
    add_command("hsplit", cmd_hsplit, 1, 0, 0, "Split: ", arg_STRING);
    add_command("inext", cmd_inext, 0, 0, 0);
    add_command("info", cmd_info, 1, 0, 0, "Format: ", arg_REST);
    ...
    add_command("link", cmd_link, 2, 1, 1,
                "Key: ", arg_STRING, "Keymap: ", arg_KEYMAP);
    add_command("listhook", cmd_listhook, 1, 1, 1, "Hook: ", arg_HOOK);
    add_command("meta", cmd_meta, 1, 0, 0, "key: ", arg_KEY);
    add_command("newkmap", cmd_newkmap, 1, 1, 1, "Keymap: ", arg_STRING);
    ...
    add_command("prompt", cmd_prompt, 1, 0, 0, "", arg_REST);
    add_command("putsel", cmd_putsel, 1, 1, 1, "Text: ", arg_RAW);
    ...
    add_command("ratwarp", cmd_ratwarp, 2, 2, 2,
                "X: ", arg_NUMBER, "Y: ", arg_NUMBER);
    add_command("ratrelwarp", cmd_ratrelwarp, 2, 2, 2,
                "X: ", arg_NUMBER, "Y: ", arg_NUMBER);
    add_command("ratclick", cmd_ratclick, 1, 0, 0, "Button: ", arg_NUMBER);
    add_command("rathold", cmd_rathold, 2, 1, 1,
                "State: ", arg_STRING, "Button: ", arg_NUMBER);
    add_command("readkey", cmd_readkey, 1, 1, 1, "Keymap: ", arg_KEYMAP);
    ...
    add_command("setenv", cmd_setenv, 2, 2, 2,
                "Variable: ", arg_STRING, "Value: ", arg_REST);
    add_command("sfdump", cmd_sfdump, 0, 0, 0);
    add_command("sfrestore", cmd_sfrestore, 1, 1, 1, "Frames: ", arg_REST);
    add_command("shrink", cmd_shrink, 0, 0, 0);
    add_command("source", cmd_source, 1, 1, 1, "File: ", arg_REST);
    add_command("smove", cmd_smove, 1, 1, 1, "Screen: ", arg_NUMBER);
    add_command("sselect", cmd_sselect, 1, 1, 1, "Screen: ", arg_NUMBER);
    ...
    add_command("unalias", cmd_unalias, 1, 1, 1, "Alias: ", arg_STRING);
    add_command("undefinekey", cmd_undefinekey, 2, 2, 2,
                "Keymap: ", arg_KEYMAP, "Key: ", arg_KEY);
    add_command("undo", cmd_undo, 0, 0, 0);
    add_command("unmanage", cmd_unmanage, 1, 1, 0, "Unmanage: ", arg_REST);
    ...
    add_command("vsplit", cmd_vsplit, 1, 0, 0, "Split: ", arg_STRING);
    add_command("windows", cmd_windows, 1, 0, 0, "", arg_REST);
...
static void clear_frame_redos(void)
{
    rp_frame_undo *cur;
...
static void del_frame_undo(rp_frame_undo *u)
{
    if (!u)
        return;
...
static void push_frame_undo(rp_vscreen *vscreen)
{
    rp_frame_undo *cur;
...
static rp_frame_undo *pop_frame_list(struct list_head *undo_list,
                                     struct list_head *redo_list)
{
...
static char *find_command_by_keydesc(char *desc, rp_keymap *map)
{
    int i = 0;
    char *keysym_name;
...
static char *resolve_command_from_keydesc(char *desc, int depth,
                                          rp_keymap *map)
{
    char *cmd, *c;
...
static void replace_keybinding(rp_action *key_action, char *newcmd)
{
...
static rp_keymap *keymap_new(char *name)
{
    rp_keymap *map;
 ...
 */
static int find_alias_index(char *name)
{
    int i;
...
static void add_alias(char *name, char *alias)
{
    int i;
...
void initialize_default_keybindings(void)
{
    ...
    add_keybinding(prefix_key.sym, prefix_key.state,
                   "readkey " ROOT_KEYMAP, top);

    /* Add keybindings to top keymap (Super + key combinations) */
    add_keybinding(XK_Down, RP_SUPER_MASK, "focusdown", map);
...
static void keymap_free(rp_keymap *map)
{
    int i;
...
static cmdret *parse_keydesc(char *keydesc, struct rp_key *key)
{
    ...
    if (!strchr(keydesc, '-')) {
        ...
        if (token == NULL) {
            ...
            free(copy);
            return cmdret_new(RET_FAILURE,
                              "parse_keydesc: Can't parse key '%s'",
                              keydesc);
        }
        do {
...
cmdret *cmd_clrunmanaged(int interactive, struct cmdarg **args)
{
...
/* Float window */
cmdret *cmd_float(int interactive, struct cmdarg **args)
{
    if (args[0] == NULL && !interactive) {
        cmdret *ret;
...
cmdret *cmd_clrfloated(int interactive, struct cmdarg **args)
{
    clear_floated_list();
    return cmdret_new(RET_SUCCESS, NULL);
}

cmdret *cmd_undefinekey(int interactive, struct cmdarg **args)
{
    cmdret *ret = NULL;
    rp_keymap *map;
    struct rp_key *key;
    ...
    if (!remove_keybinding(key->sym, key->state, map))
        ret = cmdret_new(RET_FAILURE,
                         "undefinekey: key '%s' is not bound",
                         ARG_STRING(1));
...
cmdret *cmd_definekey(int interactive, struct cmdarg **args)
{
    cmdret *ret = NULL;
    rp_keymap *map;
    struct rp_key *key;
...
cmdret *cmd_source(int interactive, struct cmdarg **args)
{
    FILE *fileptr;
...
cmdret *cmd_meta(int interactive, struct cmdarg **args)
{
    XEvent ev;
...
cmdret *cmd_prev(int interactive, struct cmdarg **args)
{
    rp_window *cur, *win;
    cur = current_window();
...
cmdret *cmd_prevframe(int interactive, struct cmdarg **args)
{
    rp_frame *frame;
...
cmdret *cmd_next(int interactive, struct cmdarg **args)
{
    rp_window *cur, *win;
    cur = current_window();
...
cmdret *cmd_nextframe(int interactive, struct cmdarg **args)
{
    rp_frame *frame;
...
cmdret *cmd_other(int interactive, struct cmdarg **args)
{
    rp_window *w;
...
static int string_to_positive_int(char *str)
{
    char *ep;
    long lval;
...
static struct list_head *trivial_completions(char *str)
{
...
static struct list_head *keymap_completions(char *str)
{
    rp_keymap *cur;
    struct list_head *list;
...
static struct list_head *window_completions(char *str)
{
    rp_window_elem *cur;
    struct list_head *list;
...
cmdret *cmd_rename(int interactive, struct cmdarg **args)
{
    if (current_window() == NULL)
        return cmdret_new(RET_FAILURE, NULL);
...
cmdret *cmd_delete(int interactive, struct cmdarg **args)
{
    XEvent ev;
    int status;
...
cmdret *cmd_kill(int interactive, struct cmdarg **args)
{
    if (current_window() == NULL)
        return cmdret_new(RET_FAILURE, NULL);
...
cmdret *cmd_version(int interactive, struct cmdarg **args)
{
    return cmdret_new(RET_SUCCESS, "%s %s", PROGNAME, VERSION);
}

static char *frame_selector(unsigned int n)
{
...
static int frame_selector_match(char ch)
{
    size_t i;
...
static cmdret *read_string(struct argspec *spec, struct sbuf *s,
...
static cmdret *read_keymap(struct argspec *spec, struct sbuf *s,
                           struct cmdarg **arg)
{
    char *input;
...
static cmdret *read_keydesc(struct argspec *spec, struct sbuf *s,
                            struct cmdarg **arg)
{
    char *input;
...
static struct list_head *vscreen_completions(char *str)
{
    struct list_head *list;
    rp_vscreen *cur;
...
static struct list_head *colon_completions(char *str)
{
    int i;
...
static cmdret *read_command(struct argspec *spec, struct sbuf *s,
                            struct cmdarg **arg)
{
    return read_string(spec, s, colon_completions, arg);
}

static struct list_head *exec_completions(char *str)
{
    size_t n = 256;
    char *partial;
    struct sbuf *line;
...
static cmdret *read_shellcmd(struct argspec *spec, struct sbuf *s,
                             struct cmdarg **arg, const char *command_name)
{
    cmdret *ret;

    ret = read_string(spec, s, exec_completions, arg);

    return ret;
}

static cmdret *read_frame(struct sbuf *s, struct cmdarg **arg)
{
    rp_frame *frame;
    ...
    if (s == NULL) {
        ...
        list_for_each_entry(cur_frame, &rp_current_vscreen->frames, node) {
            int width, height;
            char *num;
            ...
            rp_draw_string(cur_screen, wins[i], STYLE_NORMAL,
                           ...
                           FONT_ASCENT(cur_screen), num, -1, NULL, NULL);
...
static cmdret *read_window(struct argspec *spec, struct sbuf *s,
                           struct cmdarg **arg)
{
    rp_window *win = NULL;
    char *name;
...
static int parse_wingravity(char *data)
{
    int ret = -1;
...
static cmdret *read_gravity(struct argspec *spec, struct sbuf *s,
                            struct cmdarg **arg)
{
    char *input;
 ...
 */
static rp_vscreen *find_vscreen(char *str)
{
    rp_vscreen *vscreen;
    int n;
...
static cmdret *read_vscreen(struct argspec *spec, struct sbuf *s,
                            struct cmdarg **arg)
{
    char *input;
    ...
    if (s)
        ...
        input = get_input(spec->prompt, vscreen_completions);
...
static struct list_head *hook_completions(char *str)
{
    struct list_head *list;
...
static cmdret *read_hook(struct argspec *spec, struct sbuf *s,
                         struct cmdarg **arg)
{
    char *input;
...
static struct set_var *find_variable(char *str)
{
    struct set_var *cur;
...
static struct list_head *var_completions(char *str)
{
    struct list_head *list;
    struct set_var *cur;
...
static cmdret *read_variable(struct argspec *spec, struct sbuf *s,
                             struct cmdarg **arg)
{
    char *input;
...
static cmdret *read_number(struct argspec *spec, struct sbuf *s,
                           struct cmdarg **arg)
{
    char *input;
...
static cmdret *read_arg(struct argspec *spec, struct sbuf *s,
                        struct cmdarg **arg, const char *command_name)
{
    cmdret *ret = NULL;
...
static cmdret *parsed_input_to_args(int num_args, struct argspec *argspec,
                                    struct list_head *list,
 ...
 * success.
 */
static cmdret *fill_in_missing_args(struct user_command *cmd,
                                    struct list_head *list,
                                    struct list_head *args,
                                    const char *command_name)
{
...
static struct cmdarg **arg_array(struct list_head *head)
{
    int i = 0;
...
static void arg_free(struct cmdarg *arg)
{
    if (!arg)
        return;
    ...
    list_for_each_entry(uc, &user_commands, node) {
        ...
        if (result == NULL) {
            if ((interactive && list_size(&args) < uc->i_required_args) ||
                (!interactive
                 && list_size(&args) < uc->ni_required_args)) {
...
cmdret *cmd_colon(int interactive, struct cmdarg **args)
{
    cmdret *result;
    char *input;
...
cmdret *cmd_exec(int interactive, struct cmdarg **args)
{
    spawn(ARG_STRING(0), current_frame(rp_current_vscreen));
    return cmdret_new(RET_SUCCESS, NULL);
}

cmdret *cmd_execw(int interactive, struct cmdarg **args)
{
    int status = -1;
...
cmdret *cmd_execa(int interactive, struct cmdarg **args)
{
    spawn(ARG_STRING(0), NULL);
    return cmdret_new(RET_SUCCESS, NULL);
}

cmdret *cmd_execf(int interactive, struct cmdarg **args)
{
    /*
...
cmdret *cmd_execv(int interactive, struct cmdarg **args)
{
    rp_vscreen *vscreen;
...
cmdret *cmd_term(int interactive, struct cmdarg **args)
{
...
cmdret *cmd_quit(int interactive, struct cmdarg **args)
{
    kill_signalled = 1;
    return cmdret_new(RET_SUCCESS, NULL);
}
...
cmdret *cmd_windows(int interactive, struct cmdarg **args)
{
...
static void
show_window_list_with_selection(rp_screen *s, char *fmt,
                                rp_window *selected_win)
{
    ...
    list_for_each_entry(we, &rp_current_vscreen->mapped_windows, node) {
        ...
        if (delimiter
            && we->node.next != &rp_current_vscreen->mapped_windows)
...
static rp_window *get_next_valid_window(rp_window *current)
{
...
static rp_window *get_prev_valid_window(rp_window *current)
{
 ...
 */
cmdret *cmd_windowselector(int interactive, struct cmdarg **args)
{
    rp_screen *s;
        ...
        case XK_KP_Enter:
            ...
            if (selected_win) {
                ...
                if (elem) {
                    ...
                    selected_win = vscreen_last_window(rp_current_vscreen);
                    if (selected_win)
                        show_window_list_with_selection(s, fmt,
                                                        selected_win);
                    else
...
static struct desktop_app *parse_desktop_file(const char *filepath)
{
    FILE *fp;
    char line[1024];
...
static void read_desktop_dir(const char *dir, struct list_head *apps)
{
    DIR *dp;
    struct dirent *entry;
    ...
    while ((entry = readdir(dp))) {
        ...
        if (strlen(entry->d_name) < 9 ||
            strcmp(entry->d_name + strlen(entry->d_name) - 8,
                   ".desktop") != 0)
            continue;
...
static int compare_apps(const char *name1, const char *name2)
{
...
static void sort_app_list(struct list_head *apps)
{
...
static char *clean_exec_field(const char *exec)
{
...
static void
show_app_list_with_selection(rp_screen *s, struct list_head *apps,
                             struct desktop_app *selected)
{
    struct sbuf *bar_buffer;
    int mark_start = 0;
    int mark_end = 0;
 ...
 */
cmdret *cmd_applauncher(int interactive, struct cmdarg **args)
{
    ...
    if (!interactive)
        return cmdret_new(RET_FAILURE,
                          "applauncher requires interactive mode");
    ...
    while (!done) {
        ...
        case XK_j:
            ...
            if (selected->node.next != &apps)
                selected =
                    list_entry(selected->node.next, struct desktop_app,
                               node);
            show_app_list_with_selection(s, &apps, selected);
            break;
        ...
        case XK_k:
            ...
            if (selected->node.prev != &apps)
                selected =
                    list_entry(selected->node.prev, struct desktop_app,
                               node);
            show_app_list_with_selection(s, &apps, selected);
            break;
...
cmdret *cmd_abort(int interactive, struct cmdarg **args)
{
    return cmdret_new(RET_SUCCESS, NULL);
}

/* Redisplay the current window by sending 2 resize events. */
cmdret *cmd_redisplay(int interactive, struct cmdarg **args)
{
...
cmdret *cmd_escape(int interactive, struct cmdarg **args)
{
    struct rp_key *key;
    rp_action *action;
    rp_keymap *map, *top;
...
cmdret *cmd_echo(int interactive, struct cmdarg **args)
{
...
static cmdret *read_split(char *str, int max, int *p)
{
    int a, b;
...
cmdret *cmd_vsplit(int interactive, struct cmdarg **args)
{
    cmdret *ret;
    rp_frame *frame;
    int pixels;
...
cmdret *cmd_hsplit(int interactive, struct cmdarg **args)
{
    cmdret *ret;
    rp_frame *frame;
    int pixels;
...
cmdret *cmd_only(int interactive, struct cmdarg **args)
{
    push_frame_undo(rp_current_vscreen);        /* fdump to stack */
    remove_all_splits();
...
cmdret *cmd_remove(int interactive, struct cmdarg **args)
{
    rp_frame *frame;
...
cmdret *cmd_shrink(int interactive, struct cmdarg **args)
{
    push_frame_undo(rp_current_vscreen);        /* fdump to stack */
...
static resize_binding resize_bindings[] = {
    ...
    { { RESIZE_SHRINK_TO_WINDOW_KEY, RESIZE_SHRINK_TO_WINDOW_MODIFIER},
     RESIZE_TO_WINDOW },
...
cmdret *cmd_resize(int interactive, struct cmdarg **args)
{
    ...
    if (interactive && args[0] == NULL) {
        ...
        while (1) {
            ...
            if (binding->action == RESIZE_VGROW)
                resize_frame_vertically(current_frame(rp_current_vscreen),
        ...
        if (args[0] && args[1]) {
            resize_frame_horizontally(current_frame(rp_current_vscreen),
                                      ARG(0, number));
            resize_frame_vertically(current_frame(rp_current_vscreen),
                                    ARG(1, number));
        } else
...
cmdret *cmd_banish(int interactive, struct cmdarg **args)
{
    rp_screen *s;
...
cmdret *cmd_banishrel(int interactive, struct cmdarg **args)
{
    rp_screen *s = rp_current_screen;
...
cmdret *cmd_ratinfo(int interactive, struct cmdarg **args)
{
    rp_screen *s;
...
cmdret *cmd_ratrelinfo(int interactive, struct cmdarg **args)
{
    rp_screen *s;
    rp_window *rpw;
    rp_frame *f;
...
cmdret *cmd_ratwarp(int interactive, struct cmdarg **args)
{
    rp_screen *s;
...
cmdret *cmd_ratrelwarp(int interactive, struct cmdarg **args)
{
...
cmdret *cmd_ratclick(int interactive, struct cmdarg **args)
{
    int button = 1;
...
cmdret *cmd_rathold(int interactive, struct cmdarg **args)
{
    int button = 1;
...
cmdret *cmd_curframe(int interactive, struct cmdarg **args)
{
    if (interactive) {
...
cmdret *cmd_help(int interactive, struct cmdarg **args)
{
    rp_keymap *map;
    ...
    if (interactive) {
        ...
        while (i < map->actions_last && old_i < map->actions_last) {
            if (drawing_keys) {
                ...
                width = rp_text_width(s, map->actions[i].data, -1, NULL);
                if (width > max_width)
                    max_width = width;
            }
...
cmdret *set_rudeness(struct cmdarg **args)
{
    int num;
...
cmdret *cmd_gravity(int interactive, struct cmdarg **args)
{
    int gravity;
    rp_window *win;
...
static void update_gc(rp_screen *s)
{
    XGCValues gcv;
...
static cmdret *set_padding(struct cmdarg **args)
{
    ...
    if (args[0] == NULL)
        return cmdret_new(RET_SUCCESS, "%d %d %d %d",
                          ...
                          defaults.padding_right, defaults.padding_bottom);
...
static cmdret *set_ignoreresizehints(struct cmdarg **args)
{
    ...
    if (args[0] == NULL)
        return cmdret_new(RET_SUCCESS, "%d", defaults.ignore_resize_hints);
...
{
    if (args[0] == NULL)
        return cmdret_new(RET_SUCCESS, "%d", defaults.win_add_cur_vscreen);
...
cmdret *cmd_setenv(int interactive, struct cmdarg **args)
{
...
cmdret *cmd_getenv(int interactive, struct cmdarg **args)
{
    char *value;
 ...
 */
cmdret *cmd_chdir(int interactive, struct cmdarg **args)
{
    const char *dir;
 ...
 * Thanks to Gergely Nagy <algernon@debian.org> for the original patch.
 */
cmdret *cmd_unsetenv(int interactive, struct cmdarg **args)
{
 ...
 * Thanks to Gergely Nagy <algernon@debian.org> for the original patch.
 */
cmdret *cmd_info(int interactive, struct cmdarg **args)
{
    struct sbuf *buf;
    if (current_window() != NULL) {
        rp_window *win = current_window();
        ...
        if (!win_elem)
            win_elem =
                vscreen_find_window(&rp_current_vscreen->unmapped_windows,
                                    win);
 ...
 * Thanks to Gergely Nagy <algernon@debian.org> for the original patch.
 */
cmdret *cmd_lastmsg(int interactive, struct cmdarg **args)
{
    show_last_message();
    return cmdret_new(RET_SUCCESS, NULL);
}

cmdret *cmd_focusup(int interactive, struct cmdarg **args)
{
    rp_frame *frame;
...
cmdret *cmd_focusdown(int interactive, struct cmdarg **args)
{
    rp_frame *frame;
...
cmdret *cmd_focusleft(int interactive, struct cmdarg **args)
{
    rp_frame *frame;
...
cmdret *cmd_focusright(int interactive, struct cmdarg **args)
{
    rp_frame *frame;
...
cmdret *cmd_exchangeup(int interactive, struct cmdarg **args)
{
    rp_frame *frame;
...
cmdret *cmd_exchangedown(int interactive, struct cmdarg **args)
{
    rp_frame *frame;
...
cmdret *cmd_exchangeleft(int interactive, struct cmdarg **args)
{
    rp_frame *frame;
...
cmdret *cmd_exchangeright(int interactive, struct cmdarg **args)
{
    rp_frame *frame;
...
cmdret *cmd_swap(int interactive, struct cmdarg **args)
{
    rp_frame *dest_frame;
    rp_frame *src_frame;
    ...
    src_frame =
        args[1] ? ARG(1, frame) : current_frame(rp_current_vscreen);
...
cmdret *cmd_link(int interactive, struct cmdarg **args)
{
    char *cmd = NULL;
    rp_keymap *map;
...
cmdret *cmd_alias(int interactive, struct cmdarg **args)
{
...
cmdret *cmd_unalias(int interactive, struct cmdarg **args)
{
    char *tmp;
    int i;
...
cmdret *cmd_nextscreen(int interactive, struct cmdarg **args)
{
...
cmdret *cmd_prevscreen(int interactive, struct cmdarg **args)
{
    rp_screen *new_screen;
    rp_frame *new_frame;
...
cmdret *cmd_smove(int interactive, struct cmdarg **args)
{
    rp_window *w;
    rp_screen *screen;
    int new_screen;
...
cmdret *cmd_sselect(int interactive, struct cmdarg **args)
{
    int new_screen;
    rp_frame *new_frame;
    rp_screen *screen;
...
cmdret *cmd_fselect(int interactive, struct cmdarg **args)
{
...
cmdret *cmd_fdump(int interactively, struct cmdarg **args)
{
    rp_screen *screen;
    cmdret *ret;
    char *dump;
...
cmdret *cmd_frestore(int interactively, struct cmdarg **args)
{
    push_frame_undo(rp_current_vscreen);        /* fdump to stack */
    return frestore(ARG_STRING(0), rp_current_vscreen);
}

cmdret *cmd_verbexec(int interactive, struct cmdarg **args)
{
...
cmdret *cmd_addhook(int interactive, struct cmdarg **args)
{
    struct list_head *hook;
    struct sbuf *cmd;
...
cmdret *cmd_remhook(int interactive, struct cmdarg **args)
{
    struct sbuf *cmd;
...
cmdret *cmd_listhook(int interactive, struct cmdarg **args)
{
    cmdret *ret;
    struct sbuf *buffer;
    struct list_head *hook;
...
cmdret *cmd_readkey(int interactive, struct cmdarg **args)
{
    ...
    if ((key_action =
         find_keybinding(keysym, x11_mask_to_rp_mask(mod), map))) {
    ...
    ret =
        cmdret_new(RET_FAILURE, "readkey: unbound key '%s'", keysym_name);
    free(keysym_name);
    return ret;
}

cmdret *cmd_newkmap(int interactive, struct cmdarg **args)
{
    rp_keymap *map;
    ...
    if (map)
        return cmdret_new(RET_FAILURE,
                          "newkmap: keymap '%s' already exists",
                          ARG_STRING(0));
...
cmdret *cmd_delkmap(int interactive, struct cmdarg **args)
{
    ...
    if (map == root || map == top)
        return cmdret_new(RET_FAILURE,
                          "delkmap: cannot delete keymap '%s'",
                          ARG_STRING(0));
...
cmdret *cmd_set(int interactive, struct cmdarg **args)
{
    struct sbuf *scur;
    struct cmdarg *acur;
    struct list_head *iter, *tmp;
    ...
    result = parsed_input_to_args(ARG(0, variable)->nargs,
                                  ARG(0, variable)->args, &head, &arglist,
                                  &parsed_args, NULL);
    if (result)
        goto failed;
...
cmdret *cmd_sfdump(int interactively, struct cmdarg **args)
{
    ...
    list_for_each_entry(cur_screen, &rp_screens, node) {
        ...
        list_for_each_entry(cur_frame,
            ...
            frameset = frame_dump(cur_frame, cur_screen->current_vscreen);
            sbuf_concat(dump, frameset);
...
cmdret *cmd_sfrestore(int interactively, struct cmdarg **args)
{
    char *copy, *ptr, *token;
    rp_screen *screen;
...
cmdret *cmd_sdump(int interactive, struct cmdarg **args)
{
    cmdret *ret;
    struct sbuf *s;
    char *tmp;
...
cmdret *cmd_cnext(int interactive, struct cmdarg **args)
{
    ...
    if (last)
        for (win = vscreen_next_window(rp_current_vscreen, cur);
             win; win = vscreen_next_window(rp_current_vscreen, win)) {
            if (win->res_class && strcmp(cur->res_class, win->res_class)) {
...
cmdret *cmd_cprev(int interactive, struct cmdarg **args)
{
    rp_window *cur, *last, *win;
    ...
    if (last)
        for (win = vscreen_prev_window(rp_current_vscreen, cur);
             win; win = vscreen_prev_window(rp_current_vscreen, win)) {
            if (win->res_class && strcmp(cur->res_class, win->res_class)) {
                set_active_window_force(win);
                return cmdret_new(RET_SUCCESS, NULL);
            }
...
cmdret *cmd_inext(int interactive, struct cmdarg **args)
{
    rp_window *cur, *last, *win;
...
cmdret *cmd_iprev(int interactive, struct cmdarg **args)
{
    rp_window *cur, *last, *win;
...
cmdret *cmd_cother(int interactive, struct cmdarg **args)
{
...
cmdret *cmd_iother(int interactive, struct cmdarg **args)
{
    rp_window *cur, *w = NULL;
...
cmdret *cmd_undo(int interactive, struct cmdarg **args)
{
    rp_frame_undo *cur;
...
cmdret *cmd_redo(int interactive, struct cmdarg **args)
{
    rp_frame_undo *cur;
    cmdret *ret;
...
cmdret *cmd_prompt(int interactive, struct cmdarg **args)
{
    cmdret *ret;
    char *output;
    ...
    if (args[0] == NULL)
        ...
        if (prefix) {
            ...
            output = get_input(arg_str, trivial_completions);
        }
    }
...
cmdret *cmd_describekey(int interactive, struct cmdarg **args)
{
    char *keysym_name;
    rp_action *key_action;
    KeySym keysym;              /* Key pressed */
...
cmdret *cmd_dedicate(int interactive, struct cmdarg **args)
{
    rp_frame *f;
...
cmdret *cmd_putsel(int interactive, struct cmdarg **args)
{
    set_selection(ARG_STRING(0));
    return cmdret_new(RET_SUCCESS, NULL);
}

cmdret *cmd_getsel(int interactive, struct cmdarg **args)
{
    char *sel;
    cmdret *ret;
    sel = get_selection();
...
cmdret *cmd_stick(int interactive, struct cmdarg **args)
{
...
cmdret *cmd_unstick(int interactive, struct cmdarg **args)
{
    rp_window *cur = current_window();
...
cmdret *cmd_commands(int interactive, struct cmdarg **args)
{
    struct sbuf *sb;
    struct user_command *cur;
    cmdret *ret;
...
cmdret *cmd_wallpaper(int interactive, struct cmdarg **args)
{
    ...
    if (ret < 0)
        return cmdret_new(RET_FAILURE,
                          "wallpaper: failed to set wallpaper");
