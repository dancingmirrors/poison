...
static ssize_t recv_unix(int fd, char **callerbuf)
{
    int firstloop;
    char *message;
    ssize_t len, count;
...
static ssize_t send_unix(int fd, char *buf, ssize_t sz)
{
    ssize_t ret, off = 0;
...
void receive_command(void)
{
    ...
    PRINT_DEBUG(("writing back %d to command client: %s", len,
                 result + 1));
