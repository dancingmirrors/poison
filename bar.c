...
static void draw_partial_string(rp_screen * s, char *msg, int len,
                                ...
                                char *color);
static void marked_message_internal(char *msg, int mark_start,
                                    int mark_end, int bar_type);
...
void redraw_sticky_bar_text(int force)
{
    ...
    XMoveResizeWindow(dpy, s->bar_window, bar_x(s, width),
                      bar_y(s, height), width, height);

    if (force) {
        XFreePixmap(dpy, bar_pm);
        XClearWindow(dpy, s->bar_window);
        bar_pm = XCreatePixmap(dpy, s->bar_window, width,
                               FONT_HEIGHT(s) * 2, DefaultDepth(dpy,
                                                                DefaultScreen
                                                                (dpy)));
    }
    ...
    XCopyArea(dpy, bar_pm, s->bar_window, s->inverse_gc,
              0, 0, (s->width / 2) - (defaults.bar_x_padding * 2),
              FONT_HEIGHT(s), defaults.bar_x_padding,
...
static int count_lines(char *msg, int len)
{
    int ret = 1;
    int i;
...
static int max_line_length(char *msg)
{
    rp_screen *s = screen_primary();
    size_t i;
    size_t start;
    ...
    for (start = 0, i = 0; i <= strlen(msg); i++) {
        if (msg[i] == '\n' || msg[i] == '\0') {
            ...
            current_width = rp_text_width(s, msg + start, i - start, NULL);
...
static int pos_in_line(char *msg, int pos)
{
    int ret;
    int i;
...
static int line_beginning(char *msg, int pos)
{
    int ret = 0;
    int i;
                    ...
                    int y_offset, int style, char *color)
{
    rp_draw_string(s, s->bar_window, style,
                   ...
                   y_offset * FONT_HEIGHT(s), msg, len + 1, NULL, color);
}
    ...
    for (i = 0; i < msg_len; ++i) {
        ...
        if (print_reason != REASON_NONE) {
            ...
            if (print_reason & REASON_NEWLINE) {
                ...
                x_offset += rp_text_width(s, msg + start, part_len, NULL);
                start = i;
            }
 ...
 * end.
 */
static void correct_mark(int msg_len, int *mark_start, int *mark_end)
{
...
static void prepare_bar(rp_screen *s, int width, int height, int bar_type)
{
             ...
             int *width, int *height)
{
    ...
    if (mark_start == 0 || start_pos_in_line == 0)
        ...
        start = rp_text_width(s, &msg[start_line_beginning],
                              start_pos_in_line,
                              NULL) + defaults.bar_x_padding;

    end = rp_text_width(s, &msg[end_line_beginning],
                        end_pos_in_line,
                        NULL) + defaults.bar_x_padding * 2;
...
static void draw_box(rp_screen *s, int x, int y, int width, int height)
{
    XGCValues lgv;
    GC lgc;
    unsigned long mask;
...
draw_mark(rp_screen *s, char *msg, int mark_start, int mark_end)
{
    ...
    get_mark_box(msg, mark_start, mark_end, &x, &y, &width, &height);
    draw_box(s, x, y, width, height);
}

static void update_last_message(char *msg, int mark_start, int mark_end)
{
    free(last_msg);
...
void marked_message(char *msg, int mark_start, int mark_end, int bar_type)
{
